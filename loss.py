...
def L2_NORM(data):
    return torch.mean(data ** 2)

def generalization_relative_error(data_test, network):
    true_label = data_test[:, -1]
    features = data_test[:, 0:-1]
    pred_label = network(features)
    return L2_NORM(true_label - pred_label).item() / L2_NORM(true_label).item()
...
class loss:
    ...
    def get_measurements_loss(self, data_measurements):
        ...
        return L2_NORM(true_label - pred_label).item()
