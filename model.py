...
class BlackScholes_model:
    ...
    def residual(self, x, network):
        ...
        residual = grad_t_u
        grad_x_u = grad_u[:, 1]
        grad_xx_u = torch.autograd.grad(grad_x_u, x, grad_outputs=Id_tensor, create_graph=True)[0][:, 1]
        residual = residual - 0.5 * (self.sigma ** 2 * x[:, 1] ** 2 * grad_xx_u + self.mu * x[:, 1] * grad_x_u)
        return residual

    def initial_condition(self, x):
        return torch.clamp(x - 0.5, 0).reshape(-1, 1)
